...
public class MotionObjectController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (HandInteraction.cableTensionReached)
        {
            Vector3 cableDir = HandInteraction.cableDirection;

            float dot = Vector3.Dot(movement.normalized, cableDir);

            if (dot > 0)
            {
                // quitar componente que estira el cable
                movement = Vector3.ProjectOnPlane(movement, cableDir);
            }
        }
